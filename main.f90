...
!         H(i, j) = nabla_r_close_close(dr, i, j)
        H(i, j) = nabla_tx_close_close(dr, i, j)
    enddo
    enddo
    do i = 1, CooN
        H(i, i) = H(i, i) +Poten(CooR(i))
        H(2*CooN +1 -i, 2*CooN +1 -i) = H(2*CooN +1 -i, 2*CooN +1 -i) +Poten(CooR(i))
    enddo
...
subroutine proc3 
    ...
    write(file_psi, form_psi) 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0 
    write(file_ana, form_ana) 0.d0, 0.d0, 0.d0, 0.d0  
    do i = 1, 2*CooN 
!         write(file_psi, form_psi) CooR(i), (abs(H(i, j))**2.d0, j = 1, 10)
        write(file_psi, form_psi) CooR(i), (H(i, j), j = 1, 10)
!         write(file_ana, form_ana) CooR(i), (f_ana(j, CooR(i))**2.d0, j = 1, 3)
        write(file_ana, form_ana) CooR(i), (f_ana(j, CooR(i)), j = 1, 3)
