...
!     real(8), parameter :: Ra = 500.d0 
    ...
    real(8) H(1:2*CooN, 1:2*CooN), E(1:2*CooN)
    integer,       parameter :: file_psi = 101,     file_energy = 102,    file_ana = 103
    character(30), parameter :: form_psi = '(30ES)', form_energy = '(ES)', form_ana = '(6ES)'
contains
...
function nabla_tx_close_close(dx, ii, jj)
    real(8), intent(in) :: dx
    integer, intent(in) :: ii, jj
    real(8), parameter :: pi = 2.0d0*acos(0.0d0)
    real(8) :: nabla_tx_close_close, i, j
    i = dble(ii)
    j = dble(jj)
    if( ii /= jj ) then 
        nabla_tx_close_close = &
            2.d0/(i -j)**2.d0
    else
        nabla_tx_close_close = &
            pi**2.d0 /3.d0
    end if
    nabla_tx_close_close = nabla_tx_close_close &
        *(-1.d0)/dx**2.d0 &
        *(-1.d0)**(i -j)
end function nabla_tx_close_close
